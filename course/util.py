...
#参数：文档对象，表格索引
#返回：表格数据字典
# def getCourseTable(doc,index):
#     try:
#         courseTable = doc.tables[index]
#     except Exception as e:
#         print(e)
#     courseData = {}
#     courseData.setdefault('name', courseTable.cell(2, 1).text)  # 课程编号
#     courseData.setdefault('课程编号',courseTable.cell(0,1).text)    #课程编号
#     courseData.setdefault('总学时',courseTable.cell(1,1).text)    #总学时
#     courseData.setdefault('课程名称',courseTable.cell(2,1).text)    #课程名称
#     courseData.setdefault('课程类别',getCourseCategory(courseTable.cell(3,1).text))    #课程类别
#     courseData.setdefault('执笔人',courseTable.cell(4,1).text)    #执笔人
#     courseData.setdefault('先修课程',courseTable.cell(5,1).text)    #先修课程
#     courseData.setdefault('学分',courseTable.cell(0,3).text)    #学分
#     courseData.setdefault('实验学时',courseTable.cell(1,3).text)    #实验/上机学时
#     courseData.setdefault('英文名称',courseTable.cell(2,3).text)    #英文名称
#     courseData.setdefault('适用专业',courseTable.cell(3,3).text)    #适用专业
#     courseData.setdefault('审核人',courseTable.cell(4,3).text)    #审核人
#     return courseData

# def jsonToCQLjson(jsonDict):
#     cqlJson = '{'
#     for (key,value) in jsonDict.items():
#         cqlJson = cqlJson + key + ':"' + value + '",'
#     cqlJson = cqlJson[0:len(cqlJson) - 1] + '}'
#     return cqlJson
...
def createCourseObject(filePath, tableIndex):
    ...
    try:
        ...
        print(e)
    course.name = courseTable.cell(2, 1).text                               #课程名称
    ...
    course.prerequisiteCourses = courseTable.cell(5,1).text                 #先修课程
...
# 获取课程名称
# 参数：文档路径，课程表格索引=0
# def getCourseName(path,index):
#     # 创建文档对象
#     doc = Document(path)
#     # 获取课程所在表格
#     try:
#         courseTable = doc.tables[index]     #文档中第index个表格
#         courseName = courseTable.cell(2, 1).text  # 课程名称
#         return courseName
#     except Exception as e:
#         print(e)
...
if __name__ == '__main__':
    ...
    path = 'C:\\Users\\10615\Desktop\\2016版-审核完成\专业类课程-理论\\《编译原理》课程教学大纲（理论）-2016版.docx'
